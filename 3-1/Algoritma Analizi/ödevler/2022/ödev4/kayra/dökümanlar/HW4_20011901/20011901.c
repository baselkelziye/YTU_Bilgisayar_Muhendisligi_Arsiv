...
int main(){
	KISI ** kisi;
	int i,j,m,X,Y,*indegreeArray,*afterindegreeArray,*flag,**visited,*queue,front=0,rear=0,*komsuSayisi;
	int mod;
	do{
	...
	for(i=0;i<KISISIZE;i++){
		if(afterindegreeArray[i]<m)
			afterindegreeArray=indegreeEksilt(i,indegreeArray,m,kisi,flag);
	}
	for(i=0;i<KISISIZE;i++){
