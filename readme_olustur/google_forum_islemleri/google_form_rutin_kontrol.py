...
def execute_command(command):
    try:
        ...
        # Komut hata ile sonuçlanırsa, hatayı yazdır, e-posta gönder ve script'i durdur
        error_message = f"Komut hatası: {e.stderr.decode()}"
        print(error_message)
...
def update_repository():
    ...
    try:
        if execute_command("git fetch"):
            return
        if execute_command("git clean -fdx"):
            return
        if execute_command("python3 hoca_icerikleri_guncelle.py"):
            return
        if execute_command("python3 ders_icerikleri_guncelle.py"):
            return 
        os.chdir("..")
        if execute_command("python3 readme_olustur.py"):
            return
        if execute_command("git add --all"):
         return
        if execute_command('git commit -m "rutin readme güncellemesi (robot)"'):
            return
        if execute_command("git push"):
            return
    finally:
