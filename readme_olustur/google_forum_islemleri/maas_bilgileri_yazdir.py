...
for durum in df['Çalışma Durumu'].unique():
    ...
    general_avg = durum_df[['2023 Maaşı (Aylık net ortalama)', '2024 Maaşı (Aylık net ortalama)']].mean()
    ...
    field_avg = durum_df.groupby('Pozisyon Alanı')[['2023 Maaşı (Aylık net ortalama)', '2024 Maaşı (Aylık net ortalama)']].mean()
    ...
    durum_df['Tecrübe Süresi Sayısal'] = durum_df['Tecrübe Süresi'].apply(tecrube_suresi_donustur)

    # Yeniden gruplama ve ortalamaları hesaplama
    experience_avg = durum_df.groupby('Tecrübe Süresi', as_index=False)[['2023 Maaşı (Aylık net ortalama)', '2024 Maaşı (Aylık net ortalama)']].mean()
