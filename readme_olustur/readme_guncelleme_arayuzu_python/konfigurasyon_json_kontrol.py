...
def get_git_repo_url():
    ...
    try:
        ...
        git_url = subprocess.check_output(
            ["git", "config", "--get", "remote.origin.url"], cwd=current_directory
        )
...
def konfigurasyon_json_guncelle(file_path):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            data = {}
        if GITHUB_URL_ANAHTARI not in data or not isinstance(
            data[GITHUB_URL_ANAHTARI], str
        ):
        ...
        for key, default_link in ANAHTAR_VE_LINKLER.items():
            if (
                ...
                or (
                    not urllib.parse.urlparse(data[key]).scheme
                    and key != DOKUMANLAR_REPO_YOLU_ANAHTARI
                )
            ):
