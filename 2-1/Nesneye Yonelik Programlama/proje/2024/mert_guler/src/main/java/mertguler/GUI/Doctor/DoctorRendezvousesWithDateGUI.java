...
import static mertguler.GUI.Doctor.DoctorModeGUI.searchDate;
import static mertguler.GUI.Doctor.SelectDoctorGUI.doctorOfMenu;

public class DoctorRendezvousesWithDateGUI implements Initializable {
    ...
    @FXML
    public TreeTableColumn<Object, String> patientNameColumn;

    @FXML
    public TreeTableColumn<Object, Number> patientNationalIdColumn;



    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        label.setText("My Appointments For Date: " + searchDate);
        ArrayList<Rendezvous> checkedRendezvouses = doctorOfMenu.getSchedule().getSessions();

        ArrayList<Rendezvous> rendezvouses = new ArrayList<>();

        for (Rendezvous rendezvous: checkedRendezvouses){
            if (rendezvous.getDate().getMonthValue() == searchDate.getMonthValue()
                    && rendezvous.getDate().getDayOfMonth() == searchDate.getDayOfMonth()){
        ...
        patientNameColumn.setCellValueFactory(param -> {
            ...
            if (cellData != null && cellData.getValue() instanceof Rendezvous) {
                ...
                return new SimpleStringProperty(rendezvous.getPatient().getName());
            } else {
                return null;
            }
        });

        patientNationalIdColumn.setCellValueFactory(param -> {
            TreeItem<Object> cellData = param.getValue();
            if (cellData != null && cellData.getValue() instanceof Rendezvous) {
                Rendezvous rendezvous = (Rendezvous) cellData.getValue();
                return new SimpleLongProperty(rendezvous.getPatient().getNational_id());
            } else {
                return null;
            }
        });
