...
public class Kitaplik {
	...
	public void rafYazdir(int rafNo) {
		...
		if (raflar[rafNo].kitaplar.isEmpty()) {
		    System.out.println("Bu rafta kitap bulunmamaktadır.");
		} else {
		    for (int i = 0; i < raflar[rafNo].kitaplar.size(); i++) {
		        System.out.println(i + ") " + raflar[rafNo].kitaplar.get(i).getAd() +
		                "\t\t" + raflar[rafNo].kitaplar.get(i).getYazar() +
		                "\t\t" + raflar[rafNo].kitaplar.get(i).getISBN());
		    }
		}
	}
}
